...
```
This class has the potential to grow very large which will make testing difficult and possibly introduce bugs.
...
```
This approach results in each class being modular. You can swap out components at ease.
...
```
Now the service (high-level module) depends on an interface. The different notification types are sent in using dependency injection. The service does not care *how* the message gets delivered. All it needs to be satisfied is that some sender is injected.

## Over doing SOLID

Like with anything in live you can over do these principles. Some basic examples of overdoing a couple of these principles are the following:

- Single Responsibility shouldn't mean it only does one job. It just means the jobs it does are closely related. You could go to the opposite extreme of one massive bloated file by creating an excessive amount of small classes that hold one property and one method a piece.
- Being afraid to inherit anything because it might violate Liskov Substitution principle. As a result you end up avoiding inheritance entirely even for classes that should clearly be a subclass of a base class.
- Interfaces can be too segregated if there is only one method per interface (there might be legitimate cases where this is the case but generally there are at least a couple methods per interface).

## Conclusion

So, to answer the question(s) I posed at the beginning of this post. I don't think these principles are merely about gatekeeping in interviews. They are still relevant two decades on from when they were grouped together. These rules help you to think how to better structure your projects and hopefully to write better code. I don't think they get spoken about specifically when actually creating new features. If I go look at some code I've written recently I can see examples of these principles. That is also something to be aware of, you're not going to see all five principles in play together all the time. If a new feature you've implemented only contains a couple of the principles, it doesn't mean you need to shoehorn in the rest to complete the acronym. It's not the end of the world if you don't use them all (or any). Not using them does not signal your project will fail. In the long run it could come back to bite you. As your project grows you might find yourself having to come up with creative hacks to get all the classes and interfaces to play nice with one another. Being aware of these principles and using them as guides rather than laws you must obey will help you write better code and think up cleaner solutions.
